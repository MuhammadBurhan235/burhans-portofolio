...
// ─── Design tokens ────────────────────────────────────────────────
const T = {
  // Card surface (Revisi shadow agar terlihat kontras di background biru)
  cardBg: "bg-white/95 backdrop-blur-md",
  cardBorder: "border border-white/60",
  cardShadow:
    "shadow-[0_4px_12px_rgba(0,0,0,0.12)] hover:shadow-[0_6px_16px_rgba(0,0,0,0.15)]",

  // Sisa konfigurasi T dibiarkan sama
  iconBg: "bg-blue-600",
  iconText: "text-white",
...
const DEFAULT_CITIES: CityOption[] = [
  {
    ...
    query: "Kota Tangerang",
    preferExactUpper: "KOTA TANGERANG",
  },
  {
    id: "",
    lokasi: "Kota Bandung",
    query: "Kota Bandung",
    preferExactUpper: "KOTA BANDUNG",
  },
];
    ...
    (async () => {
      try {
        const res = await fetch(
          `${SHOLAT_API_BASE}/kota/cari/${encodeURIComponent(cityQuery)}`,
        );
        ...
        const hit =
          data.find(
            (c: any) => c?.lokasi?.trim().toUpperCase() === preferExactUpper,
          ) ?? data[0];
        if (!dead) {
          setCityId(hit?.id ? String(hit.id) : null);
          setCityName(hit?.lokasi ?? cityQuery);
        }
      } catch {
        if (!dead) {
          setError("Gagal memuat kota");
          setCityId(null);
          setCityName("");
        }
      } finally {
    ...
    (async () => {
      try {
        ...
        const [r1, r2] = await Promise.all([
          fetch(
            `${SHOLAT_API_BASE}/jadwal/${cityId}/${t.year}/${t.month}/${t.day}`,
          ),
          fetch(
            `${SHOLAT_API_BASE}/jadwal/${cityId}/${tm.year}/${tm.month}/${tm.day}`,
          ),
        ]);
        if (!r1.ok || !r2.ok) throw new Error();
        const [j1, j2] = await Promise.all([r1.json(), r2.json()]);
        if (!dead) {
          setToday(j1?.data?.jadwal ?? null);
          setTmrw(j2?.data?.jadwal ?? null);
        }
      } catch {
        if (!dead) {
...
export default function SholatCountdownMini({
  ...
  const city0 = useCitySchedule(
    DEFAULT_CITIES[0].query,
    DEFAULT_CITIES[0].preferExactUpper,
  );
  const city1 = useCitySchedule(
    DEFAULT_CITIES[1].query,
    DEFAULT_CITIES[1].preferExactUpper,
  );
  ...
  const nextPrimary = useMemo(() => {
    ...
    return (
      findNextPrayer(primary.scheduleToday, today) ??
      findNextPrayer(primary.scheduleTmrw, tmrw)
    );
  ...
  const nextSel = useMemo(() => {
    ...
    return (
      findNextPrayer(sel.scheduleToday, today) ??
      findNextPrayer(sel.scheduleTmrw, tmrw)
    );
    ...
    <LayoutGroup id="sholat-widget">
      <div ref={containerRef} className="relative">
        <motion.div
          ...
          // REVISI: Mode mini = kapsul (9999), mode expanded = kotak membulat (24)
          ...
          className={[
            ...
            expanded
              ...
              : "cursor-pointer hover:bg-white select-none",
            "transition-colors duration-200",
          ].join(" ")}
        >
          {/* ─── Shared Header ─── */}
          {/* REVISI: Padding saat mini (px-2.5 py-1.5) dibuat mirip tombol navbar */}
          <div
            className={`flex items-center gap-2.5 ${expanded ? "px-4 pt-4 pb-3" : "px-2.5 py-1.5"}`}
          >
            <motion.div
              ...
              className={[
                ...
                expanded
                  ? "w-10 h-10"
                  : "w-8 h-8 max-[536px]:w-6 max-[536px]:h-6",
              ].join(" ")}
            >
            ...
            <div className="flex flex-col leading-tight flex-1 min-w-0 gap-1">
              ...
              <motion.div
                layoutId="sholat-city"
                transition={spring}
                className={`items-center gap-1 ${expanded ? "flex" : "hidden min-[528px]:flex"}`}
              >
                <FaMapMarkerAlt
                  className={`shrink-0 ${T.pinColor} ${expanded ? "text-[11px]" : "text-[10px]"}`}
                />
                <motion.span
                ...
                >
                  {expanded
                    ? cities[selectedCityIdx].cityName ||
                      DEFAULT_CITIES[selectedCityIdx].lokasi
                  ...
                  {!primary.loading &&
                    ...
                    countdownPrimary && (
                      <div className="flex items-center gap-1.5">
                        ...
                        >
                          {nextPrimary.name}
                          <span className={`ml-1 font-semibold ${T.subText}`}>
                            {nextPrimary.time}
                          </span>
                        </motion.span>
                        <motion.span
                      ...
                      {PRAYER_KEYS.map((p) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span
                              className={`text-[10px] font-semibold mb-1 ${isNext ? "text-blue-100" : T.subText}`}
                            >
                              {p.label}
                            </span>
                            {/* Time — same weight as prayer name in mini */}
                            <span
                              className={`text-[13px] font-bold ${isNext ? "text-white" : T.prayerLabel}`}
                            >
                              {timeStr}
                            </span>
                            {isNext && (
                ...
                {!sel.loading && !sel.error && nextSel && countdownSel && (
                  <div className="mx-4 mb-4 rounded-xl bg-blue-600 px-4 py-3 flex items-center justify-between shadow-[0_4px_12px_rgba(8,74,131,0.28)]">
                    <div className="flex flex-col gap-0.5">
                      ...
                      >
                        {nextSel.name}
                        <span className="ml-1.5 text-blue-200 font-semibold">
                          {nextSel.time}
                        </span>
                      </motion.span>
